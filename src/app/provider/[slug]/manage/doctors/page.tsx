...
export async function generateMetadata({ params }: PageProps<"/provider/[slug]/manage/doctors">): Promise<Metadata> {
  const { slug } = await params;
  return getProviderManageMetadata(decodeURIComponent(slug), "Doctors");
}

export default async function ProviderManageDoctorsPage({ params }: PageProps<"/provider/[slug]/manage/doctors">) {
  const { slug } = await params;
