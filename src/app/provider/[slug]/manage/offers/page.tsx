...
export async function generateMetadata({ params }: PageProps<"/provider/[slug]/manage/offers">): Promise<Metadata> {
  const { slug } = await params;
  return getProviderManageMetadata(decodeURIComponent(slug), "Offers");
}

export default async function ProviderOffersPage({ params }: PageProps<"/provider/[slug]/manage/offers">) {
  const { slug } = await params;
