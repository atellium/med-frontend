...
export async function generateMetadata({ params }: PageProps<"/[slug]/manage/info">): Promise<Metadata> {
  const { slug } = await params;
  return getProviderManageMetadata(decodeURIComponent(slug), "Provider Information");
}

export default async function ProviderInfoPage({ params }: PageProps<"/[slug]/manage/info">) {
  const { slug } = await params;
