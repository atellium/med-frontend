...
export async function generateMetadata({ params }: PageProps<"/[slug]/manage/products">): Promise<Metadata> {
  const { slug } = await params;
  return getProviderManageMetadata(decodeURIComponent(slug), "Products");
}

export default async function ProviderManageProductsPage({ params }: PageProps<"/[slug]/manage/products">) {
  const { slug } = await params;
