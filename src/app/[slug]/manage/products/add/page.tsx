...
export async function generateMetadata({ params }: PageProps<"/[slug]/manage/products/add">): Promise<Metadata> {
  const { slug } = await params;
  return getProviderManageMetadata(decodeURIComponent(slug), "Add Product");
}

export default async function AddProductPage({ params }: PageProps<"/[slug]/manage/products/add">) {
  const { slug } = await params;
