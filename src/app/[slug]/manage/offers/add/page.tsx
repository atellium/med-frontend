...
export async function generateMetadata({ params }: PageProps<"/[slug]/manage/offers/add">): Promise<Metadata> {
  const { slug } = await params;
  return getProviderManageMetadata(decodeURIComponent(slug), "Add Offer");
}

export default async function AddOfferPage({ params }: PageProps<"/[slug]/manage/offers/add">) {
  const { slug } = await params;
